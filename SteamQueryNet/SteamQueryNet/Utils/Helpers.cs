...
internal class Helpers
{
    ...
    internal static (string serverAddress, ushort port) ResolveIPAndPortFromString(string serverAddressAndPort)
    {
        ...
        if (string.IsNullOrEmpty(serverAddressAndPort))
            throw new ArgumentException($"Couldn't parse hostname or port with value: {serverAddressAndPort}",
                nameof(serverAddressAndPort));
        ...
        if (!ushort.TryParse(parts[1], out var parsedPort))
            throw new ArgumentException(
                $"Couldn't parse the port number from the parameter with value: {serverAddressAndPort}",
                nameof(serverAddressAndPort));
