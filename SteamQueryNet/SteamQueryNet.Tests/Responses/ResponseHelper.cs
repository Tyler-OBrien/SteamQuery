...
internal sealed class ResponseHelper
{
    ...
    private static readonly Dictionary<string, object> _responses = new()
    {
        {
            ServerInfo, new ServerInfo
            {
                ...
                ID = 730,
                Keywords =
                    "!knife,!ws,64tick,SurfHeaven,autobhop,cs20,knife,rank,skins,stats,surf,surfing,surftimer,timer,ws,secure",
    ...
    public static (byte[], object) GetValidResponse(string responseType)
    {
        ...
        if (!_responses.TryGetValue(responseType, out var responseObject))
            throw new ArgumentException($"Invalid response type received: {responseType}." +
                                        $"Consider registering {responseType} into ResponseHelpers dictionary.",
                nameof(responseType));
